...
def write_file(time, freq):
    ...
    i = 0
    while i < total:
        deltatime.append(round(i*(1/freq), 4))
        i = i + 1    
    better_data_list = list(chain.from_iterable(zip(deltatime, comma, accelX, comma, accelY, comma, accelZ, comma, gyroX, comma, gyroY, comma, gyroZ, comma, magX, comma, magY, comma, magZ, enter))))
    data_file.writelines(better_data_list)
    sensor_data = open(data/sensor_data.csv)
    sensor_data.writelines(better_data_list)
    data_file.close()
